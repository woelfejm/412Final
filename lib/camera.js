...
var Camera = function(aspect) {
    ...
    this.far = 1000.0;             // The distance to the far plane
...
Camera.prototype.dolly = function(delta) {
    let d = delta * 0.1;
    let v = vec3.fromValues(this.rotation[2], this.rotation[6], this.rotation[10]);
    vec3.scaleAndAdd(this.eye, this.eye, v, d);
};
...
/**
 * This method sets the camera to follow behind the car c and look in the direction the car is facing.
 * @param {} c - car object the camera is following 
 */
Camera.prototype.thirdPerson = function(c){
    let camLoc = vec3.fromValues(c.pos[0] + Math.sin(c.direction) * -20, c.pos[1] + 5, c.pos[2] + Math.cos(c.direction) * -20);
    camera.lookAt(camLoc, c.pos, vec3.fromValues(0,1,0));
}

Camera.prototype.reverse = function(c){
    let camLoc = vec3.fromValues(c.pos[0] + Math.sin(c.direction) * 10, c.pos[1] + 5, c.pos[2] + Math.cos(c.direction) * 10);
    camera.lookAt(camLoc, c.pos, vec3.fromValues(0,1,0));
}

Camera.prototype.firstPerson = function(c){
    let camLoc = vec3.fromValues(c.pos[0] - Math.sin(c.direction), c.pos[1] + 4, c.pos[2] - Math.cos(c.direction));
    let camDir = vec3.fromValues( c.pos[0] + Math.sin(c.direction) * 5, c.pos[1] + 4, c.pos[2] + Math.cos(c.direction) * 5 )
    camera.lookAt(camLoc, camDir, vec3.fromValues(0,1,0));
}
