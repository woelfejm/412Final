...
var car = function(scale = 3){
    ...
    this.accelerationRate = 1/(scale * 70);                        //the rate to increase speed by if gas 
...
car.prototype.updateModel  = function(){
    ...
    /*
    this.sphLF[0] = this.sphLF[0] + (this.speed * Math.sin(this.direction));  //left front
    this.sphLF[2] = this.sphLF[2] + (this.speed * Math.cos(this.direction));
    
    this.sphRF[0] = this.sphRF[0] + (this.speed * Math.sin(this.direction));  //right front
    this.sphRF[2] = this.sphRF[2] + (this.speed * Math.cos(this.direction));

    this.sphLR[0] = this.sphLR[0] + (this.speed * Math.sin(this.direction));  //left rear
    this.sphLR[2] = this.sphLR[2] + (this.speed * Math.cos(this.direction));

    this.sphRR[0] = this.sphRR[0] + (this.speed * Math.sin(this.direction));  //right rear
    this.sphRR[2] = this.sphRR[2] + (this.speed * Math.cos(this.direction));
    //currently using central pos
     */
    ...
    //console.log("lf:"+sphLF);
    //console.log("rf:"+sphRF);
...
car.prototype.checkCollision = function(spheres,rad){
    ...
    for(var i = 0; i < spheres.length; i++){
        ...
        var tempAngle;
        var angleOut;
        //console.log(Math.sqrt(Math.pow(this.pos[0]-spheres[i][0],2) + Math.pow(this.pos[2]-spheres[i][2],2)));
        if(this.sphRad + rad > Math.sqrt(Math.pow(this.sphLF[0]-spheres[i][0],2) + Math.pow(this.sphLF[2]-spheres[i][2],2))){
            ...
            //travelVec = this.pos - this.lastpos;
            vec3.subtract(collisionVec, this.pos, spheres[i]);
           // uVec = cross(travelVec, collisionVec);
            //newTravelVec = cross(uVec, collisionVec);
            tempAngle = vec3.angle( collisionVec, travelVec);
            angleOut = Math.PI - tempAngle * 2;
        ...
        }/*else if(this.sphRad + rad > Math.sqrt(Math.pow(this.sphLR[0]-spheres[i][0],2) + Math.pow(this.sphLR[2]-spheres[i][2],2))){
            console.log("collision LR");
        }else if(this.sphRad + rad > Math.sqrt(Math.pow(this.sphRR[0]-spheres[i][0],2) + Math.pow(this.sphRR[2]-spheres[i][2],2))){
            console.log("collision RR");
        }*/
